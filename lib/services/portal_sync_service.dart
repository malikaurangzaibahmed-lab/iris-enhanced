...
import 'session_refresher_service.dart';
...
class PortalSyncService {
  ...
  static const String _lastSyncStatusKey =
      'portal_last_bg_sync_status'; // 'success' or 'failed'
  static const String _syncCooldownKey = 'portal_bg_sync_cooldown';
  static const int _defaultCooldown = 15 * 60 * 1000; // 15 mins
  ...
  static Future<List<PortalTask>> performBackgroundSync({
    bool force = false,
  }) async {
    ...
    try {
      ...
      var cookieHeader = prefs.getString(
        'iris_session_student_${host.toLowerCase()}_cookies',
      );

      if (cookieHeader == null || cookieHeader.isEmpty) {
        final warmed = await SessionRefresherService.warmSession(
          host,
          'student',
        );
        if (warmed) {
          cookieHeader = prefs.getString(
            'iris_session_student_${host.toLowerCase()}_cookies',
          );
        }
      }
      ...
      if (cookieHeader == null || cookieHeader.isEmpty) {
        await prefs.setString(_lastSyncStatusKey, 'expired');
        notifyUpdate();
        return [];
      }
      ...
      var dashResp = await http.get(
        Uri.parse('https://$host/Courses/Index'),
        headers: headers,
      );
      if (dashResp.statusCode != 200) {
        ...
        if (warmed) {
          final newCookieHeader = prefs.getString(
            'iris_session_student_${host.toLowerCase()}_cookies',
          );
          if (newCookieHeader != null && newCookieHeader.isNotEmpty) {
            headers['Cookie'] = newCookieHeader;
            dashResp = await http.get(
              Uri.parse('https://$host/Courses/Index'),
              headers: headers,
            );
          }
      ...
      for (final course in courseData) {
        ...
        final assignResp = await http.get(
          Uri.parse('https://$host/Assignments/Index'),
          headers: headers,
        );
        if (assignResp.statusCode == 200) {
          // Offload Assignment Parsing to Isolate
          final tasks = await compute(
            _parsePortalTableInIsolate,
            _ParserInput(assignResp.body, courseName, 'Assignment'),
          );
          results.addAll(tasks);
        }

        var quizResp = await http.get(
          Uri.parse('https://$host/Course/Quizzes'),
          headers: headers,
        );
        if (quizResp.statusCode != 200) {
          quizResp = await http.get(
            Uri.parse('https://$host/Quizzes/Index'),
            headers: headers,
          );
        }
        ...
        if (quizResp.statusCode == 200) {
          ...
          final tasks = await compute(
            _parsePortalTableInIsolate,
            _ParserInput(quizResp.body, courseName, 'Quiz'),
          );
          results.addAll(tasks);
        }
      }
    ...
    for (final table in tables) {
      ...
      for (final row in rows) {
        ...
        if (hasAction) {
          ...
          String rawDate = cells.length > 3
              ? cells[3].text.trim()
              : cells.last.text.trim();
          if (input.type == 'Quiz' && cells.length > 2)
          ...
          if (title.toLowerCase().contains('no data') || title.length < 2)
            continue;
  ...
  static Future<void> _updateStoredTasks(
    List<PortalTask> newTasks,
    ...
    try {
      ...
      for (final et in existingTasks) {
        final key =
            '${et.subject.trim().toLowerCase()}_${et.title.trim().toLowerCase()}_${et.type.trim().toLowerCase()}';
      ...
      for (final nt in newTasks) {
        final key =
            '${nt.subject.trim().toLowerCase()}_${nt.title.trim().toLowerCase()}_${nt.type.trim().toLowerCase()}';
      ...
      await prefs.setString(
        _cachedTasksKey,
        jsonEncode(mergedList.map((t) => t.toJson()).toList()),
      );
