...
class RemoteConfigService {
  ...
  static final ValueNotifier<Map<String, dynamic>?> liveAnnouncement =
      ValueNotifier<Map<String, dynamic>?>(null);
  static final ValueNotifier<DateTime?> lastConfigUpdateTime =
      ValueNotifier<DateTime?>(null);
  static final ValueNotifier<DateTime?> lastTimetableUpdateTime =
      ValueNotifier<DateTime?>(null);
  static final ValueNotifier<List<dynamic>> midtermExams =
  ...
  static String formatTimestamp(DateTime? dateTime) {
    ...
    final hourVal = localDateTime.hour == 0
        ? 12
    ...
    _configSubscription = FirebaseFirestore.instance
        ...
        .listen(
          (DocumentSnapshot doc) async {
            ...
            final remotePeriod =
                data['academic_period']?.toString() ?? 'classes';
            final prefs = await SharedPreferences.getInstance();
            ...
            final remoteTimetableVersion =
                (data['active_timetable_version'] as num?)?.toInt() ?? 0;
            final remoteTimetableJson =
                data['active_timetable_json']?.toString() ?? '';

            if (remoteTimetableVersion > 0) {
              if (remoteTimetableVersion.toString().length == 10) {
                lastTimetableUpdateTime.value =
                    DateTime.fromMillisecondsSinceEpoch(
                      remoteTimetableVersion * 1000,
                    );
              } else {
                lastTimetableUpdateTime.value =
                    DateTime.fromMillisecondsSinceEpoch(remoteTimetableVersion);
              }
            }

            if (remoteTimetableJson.isNotEmpty) {
              final prefs = await SharedPreferences.getInstance();
              final currentTimetableVersion =
                  prefs.getInt(TimetableOTAService.prefTimetableVersion) ?? 0;

              if (remoteTimetableVersion > currentTimetableVersion) {
                debugPrint(
                  '📥 IRIS Remote Engine: Downloading remote timetable seed upgrade (v$remoteTimetableVersion)...',
                );
                try {
                  ...
                  await prefs.setInt(
                    TimetableOTAService.prefTimetableVersion,
                    remoteTimetableVersion,
                  );
                  await prefs.setInt(
                    TimetableOTAService.prefLastCheckTime,
                    remoteTimetableVersion,
                  );

                  debugPrint(
                    '✅ IRIS Remote Engine: Saved timetable OTA data ($sessionCount sessions)',
                  );
                  ...
                  if (context.mounted) {
                    showIrisFrostedSnackBar(
                      context,
                      content: const Text(
                        '✅ Remote Timetable sync complete! Rebuilding schedule...',
                      ),
            ...
            if (updateData != null) {
              final showUpdate =
                  updateData['show_update_card'] as bool? ?? true;
              final vName = updateData['version_name']?.toString() ?? '';
              final dismissedTag =
                  prefs.getString('dismissed_admin_update_tag') ?? '';
              final isDismissed =
                  (vName.isNotEmpty &&
                      (prefs.getBool('dismissed_admin_update_$vName') ??
                          false)) ||
                  (dismissedTag.isNotEmpty &&
                      vName.isNotEmpty &&
                      (dismissedTag == vName ||
                          dismissedTag.contains(vName) ||
                          vName.contains(dismissedTag)));
            ...
            final broadcastEnabled =
                data['broadcast_enabled'] as bool? ?? false;
            ...
            final midtermJsonRaw =
                data['active_midterm_json']?.toString() ?? '';
            if (midtermJsonRaw.isNotEmpty) {
              try {
                ...
                debugPrint(
                  '❌ IRIS Remote Engine: Failed to parse active_midterm_json: $e',
                );
              }
            } else {
              midtermExams.value = [];
            ...
            if (finalsJsonRaw.isNotEmpty) {
              try {
                ...
                debugPrint(
                  '❌ IRIS Remote Engine: Failed to parse active_finals_json: $e',
                );
              }
            } else {
              finalExams.value = [];
  ...
  static Color _getModeColor(String period) {
    switch (period) {
      case 'classes':
        return const Color(0xFF3A86FF); // Brand blue
      case 'midterms':
  ...
  static List<String> _calculateTimetableDiffs(
    ...
    try {
      ...
      bool matchesBatch(dynamic item) {
        ...
        final batchVal =
            (item['batch'] ?? item['class_name'] ?? item['section'] ?? '')
                .toString()
                .trim()
                .toLowerCase();
      ...
      String getDay(Map<String, dynamic> item) =>
          (item['day'] ?? item['weekday'] ?? 'Monday').toString();
      String getSubject(Map<String, dynamic> item) =>
          (item['subject'] ?? item['course'] ?? item['title'] ?? 'Unknown')
              .toString();
      String getRoom(Map<String, dynamic> item) =>
          (item['room'] ?? item['location'] ?? 'TBD').toString();
      String getTeacher(Map<String, dynamic> item) =>
          (item['teacher'] ?? item['instructor'] ?? item['staff'] ?? 'Unknown')
              .toString();
      ...
      for (final oldItem in oldSessions) {
        ...
        if (match.isEmpty) {
          ...
          if (oldRoom.toLowerCase() != newRoom.toLowerCase()) {
            diffs.add(
              'Room changed: $oldSubj now in $newRoom (was $oldRoom) on $oldDay at $oldTime',
            );
          }
          ...
          if (oldTeach.toLowerCase() != newTeach.toLowerCase()) {
            diffs.add(
              'Instructor changed: $oldSubj now taught by $newTeach on $oldDay',
            );
          }
        }
      }
      ...
      for (final newItem in newSessions) {
        ...
        if (!exists) {
          ...
          diffs.add(
            'New class added: $newSubj in $newRoom on $newDay at $newTime',
          );
        }
      }
    } catch (e) {
